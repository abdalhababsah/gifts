...
@section('content')
    <div
        ...
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            ...
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                <div class="grow">
                    <h5 class="text-16">Brands Management</h5>
                </div>
                <ul class="flex items-center gap-2 text-sm font-normal shrink-0">
                    ...
                    <li class="text-slate-700 dark:text-zink-100">Brands</li>
                </ul>
            </div>
                    ...
                    <div class="card-body">
                        <div class="flex items-center gap-4">
                            ...
                            <div class="grow">
                                ...
                                <p class="text-slate-500 dark:text-zink-200">Total Brands</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div class="flex items-center gap-4">
                            ...
                            <div class="grow">
                                ...
                                <p class="text-slate-500 dark:text-zink-200">Active Brands</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div class="flex items-center gap-4">
                            ...
                            <div class="grow">
                                ...
                                <p class="text-slate-500 dark:text-zink-200">Inactive Brands</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="flex items-center">
                        <h6 class="text-15 grow">Brands List</h6>
                        <div class="shrink-0">
                            <button type="button" data-modal-target="addBrandModal"
                            ...
                            <div class="grid grid-cols-1 gap-5 xl:grid-cols-12">
                                <div class="relative xl:col-span-4">
                                    <input type="text" name="search"
                                        ...
                                        placeholder="Search brands..." autocomplete="off" value="{{ request('search') }}">
                        ...
                        <table class="w-full border-separate table-custom border-spacing-y-1 whitespace-nowrap">
                            <thead class="text-left">
                                <tr
                                    ...
                                    <th class="px-3.5 py-2.5 first:pl-5 last:pr-5 font-semibold">Brand ID</th>
                                    ...
                                    <tr class="relative rounded-md after:absolute ltr:after:border-l-2 rtl:after:border-r-2 ltr:after:left-0 rtl:after:right-0 after:top-0 after:bottom-0 after:border-transparent [&.active]:after:border-custom-500 [&.active]:bg-slate-100 dark:[&.active]:bg-zink-600"
                                        ...
                                        <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 brand-image">
                                            <div
                                                ...
                                                @if ($brand->image_path)
                                                    <img src="{{ Storage::url($brand->image_path) }}"
                                                        alt="{{ $brand->name_en }}"
                                        ...
                                        <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 font-medium name-en">
                                            {{ $brand->name_en }}</td>
                                        <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 name-ar">{{ $brand->name_ar }}</td>
                                        <td class="px-3.5 py-2.5 first:pl-5 last:pr-5">
                                            <span
                                                class="px-2.5 py-0.5 text-xs font-medium rounded border bg-slate-100 border-slate-200 text-slate-500 dark:bg-slate-500/20 dark:border-slate-500/20 dark:text-zink-200">
                                                {{ $brand->slug }}
                                            </span>
                                        </td>
                                        <td class="px-3.5 py-2.5 first:pl-5 last:pr-5">
                                        ...
                                        <td class="px-3.5 py-2.5 first:pl-5 last:pr-5">
                                            <div class="status-toggle-container" data-id="{{ $brand->id }}">
                                                <button type="button"
                                                    class="status-toggle cursor-pointer inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $brand->is_active ? 'bg-green-100 text-green-500 dark:bg-green-500/20 dark:border-transparent' : 'bg-red-100 text-red-500 dark:bg-red-500/20 dark:border-transparent' }}"
                                                    data-status="{{ $brand->is_active ? 1 : 0 }}"
                                                    data-id="{{ $brand->id }}"
                                                    data-name="{{ $brand->name_en }}"
                                                    data-url="{{ route('admin.brands.toggleStatus', $brand) }}"
                                                    onclick="confirmToggleStatus(this)">
                                                    <i data-lucide="{{ $brand->is_active ? 'check-circle' : 'x-circle' }}" class="status-icon size-3 mr-1.5"></i>
                                                    <span class="status-text">{{ $brand->is_active ? 'Active' : 'Inactive' }}</span>
                                                </button>
                                            </div>
                                        </td>
                                        <td class="px-3.5 py-2.5 first:pl-5 last:pr-5">
                                                    ...
                                                    <li>
                                                        <form action="{{ route('admin.brands.destroy', $brand) }}"
                                                            ...
                                                            <button type="submit"
                                                                ...
                                                                onclick="return confirm('Are you sure you want to delete {{ addslashes($brand->name_en) }}? This action cannot be undone.')">
    ...
    {{-- Brand Add Modal --}}
    <div id="addBrandModal" modal-center
        class="fixed flex flex-col hidden transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4 show">
        <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
            <div class="flex items-center justify-between p-4 border-b dark:border-zink-300/20">
                <h5 class="text-16" id="modalTitle">Add Brand</h5>
                <button data-modal-close="addBrandModal"
                    class="transition-all duration-200 ease-linear text-slate-400 hover:text-red-500"><i data-lucide="x"
                        class="size-5"></i></button>
            </div>
            <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
                {{-- Create Brand Form --}}
                <form id="createBrandForm" action="{{ route('admin.brands.store') }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                        <label class="inline-block mb-2 text-base font-medium">Brand Image</label>
                        <div class="flex items-center gap-2">
                            <div class="relative">
                                <div id="createImagePreview"
                                    class="size-14 rounded-md bg-slate-100 dark:bg-zink-600 border border-dashed flex items-center justify-center overflow-hidden">
                                    <i data-lucide="image" class="size-4 text-slate-400" id="createImagePlaceholder"></i>
                                    <img id="createPreviewImage" src="" alt="Preview"
                                        class="size-full object-cover rounded-md hidden">
                                </div>
                                <button type="button" onclick="document.getElementById('create_image').click()"
                                    class="absolute -top-1 -right-1 size-4 bg-custom-500 text-white rounded-full flex items-center justify-center">
                                    <i data-lucide="pencil" class="size-2"></i>
                                </button>
                            </div>
                            <div class="flex-1">
                                <input type="file" id="create_image" name="image" accept="image/*" class="hidden"
                                    onchange="previewImageFile(this, 'create')">
                    ...
                    <div class="mb-3">
                        <label for="create_name_en" class="inline-block mb-2 text-base font-medium">Name (EN) <span
                    ...
                    <div class="mb-3">
                        <label for="create_name_ar" class="inline-block mb-2 text-base font-medium">Name (AR) <span
                                class="text-red-500">*</span></label>
                    ...
                    <div class="flex justify-end gap-2 mt-4">
                        <button type="reset" data-modal-close="addBrandModal"
                ...
                {{-- Edit Brand Form --}}
                <form id="editBrandForm" action="" method="POST" class="hidden" enctype="multipart/form-data">
                    @csrf
                    {{-- Do not add @method('PUT') here as route uses POST --}}
                    <div class="mb-3">
                        <label class="inline-block mb-2 text-base font-medium">Brand Image</label>
                        <div class="flex items-center gap-2">
                            <div class="relative">
                                <div id="editImagePreview"
                                    class="size-14 rounded-md bg-slate-100 dark:bg-zink-600 border border-dashed flex items-center justify-center overflow-hidden">
                                    <i data-lucide="image" class="size-4 text-slate-400" id="editImagePlaceholder"></i>
                            ...
                            <div class="flex-1">
                                <input type="file" id="edit_image" name="image" accept="image/*" class="hidden"
                                    onchange="previewImageFile(this, 'edit')">
                                <p class="text-xs text-slate-500">Max: 2MB</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        <label for="edit_name_en" class="inline-block mb-2 text-base font-medium">Name (EN) <span
                                class="text-red-500">*</span></label>
                    ...
                    <div class="mb-3">
                        <label for="edit_name_ar" class="inline-block mb-2 text-base font-medium">Name (AR) <span
                                class="text-red-500">*</span></label>
...
@push('scripts')
    <script>
        ...
        function openCreateModal() {
            ...
            document.getElementById('createBrandForm').classList.remove('hidden');
            ...
            document.getElementById('createPreviewImage').classList.add('hidden');
            document.getElementById('createPreviewImage').src = '';
        ...
        function openEditModal(brandId) {
            ...
            const imagePath = row.querySelector('.brand-image img')?.getAttribute('src') || '';

            // Set form action
            const editForm = document.getElementById('editBrandForm');
            editForm.action = `{{ url('admin/brands') }}/${brandId}`;
            
            // Remove any existing _method input fields to prevent PUT method being sent
            const methodInputs = editForm.querySelectorAll('input[name="_method"]');
            ...
            if (imagePath) {
                ...
                document.getElementById('editPreviewImage').classList.remove('hidden');
                document.getElementById('editPreviewImage').src = imagePath;
            } else {
            ...
            document.getElementById('createBrandForm').classList.add('hidden');
        ...
        function previewImageFile(input, prefix) {
            if (input.files && input.files[0]) {
                ...
                reader.onload = function(e) {
                    document.getElementById(`${prefix}ImagePlaceholder`).classList.add('hidden');
                    document.getElementById(`${prefix}PreviewImage`).classList.remove('hidden');
                    document.getElementById(`${prefix}PreviewImage`).src = e.target.result;
                }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelector('[data-modal-target="addBrandModal"]').addEventListener('click', function(e) {
                e.preventDefault();
                openCreateModal();
            });

            // Close modal buttons
            document.querySelectorAll('[data-modal-close="addBrandModal"]').forEach(button => {
        ...
        @if ($errors->any() && request()->is('*/brands/*') && !request()->isMethod('get'))
            document.addEventListener('DOMContentLoaded', function() {
                // Get the brand ID from the form action in old input
                const actionUrl = "{{ old('_action') }}";
                const brandId = actionUrl ? actionUrl.split('/').pop() : null;
                if (brandId) {
